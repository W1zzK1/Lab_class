...
void Account::changeOwnerSurname(string ownerSurname) {
	this->ownerSurname = ownerSurname;
};
string Account::getOwnerSurname() {
	return this->ownerSurname;
}
int Account::getAmountRubles() {
	return this->amountRubles;
}
float Account::getAccrualPercentage() {
...
void Account::withdrawMoneyFromAccount(int amount) {
	if (amountRubles == 0) {
...
void Account::depositMoneyToAccount(int amount) {
...
void Account::accruePercentage(float percentage) {
	accrualPercentage += percentage;
}
int Account::convertAmountToDollars() {
...
int Account::convertAmountInEuros() {
	if (amountRubles > 9999) {
		cout << "Недопустимая сумма баланса" << endl;
	}
	else {
